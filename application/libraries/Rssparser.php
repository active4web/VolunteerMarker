	...
	function parse()
	{
		...
		if ($xml->channel)
		{
			...
			foreach ($xml->channel->item as $item)
			{
				...
				$def_img = 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/300px-No_image_available.svg.png';
				$data = array();
				if(!empty($enclosure)){$thumb = $enclosure;}
				elseif(!empty($image)){$thumb = $image;}
				else{$thumb = $def_img;}
				$data['title'] = (string)$item->title;
				$data['description'] = (string)$item->description;
				$data['enclosure'] = $thumb;
